...
document.querySelector('#delete').addEventListener('click', async event => {
  if (!token || !confirm('Permanently delete this account and all saved conversations? This cannot be undone.')) return;
...
setup().catch(error => { token = null; form.hidden = true; panel.hidden = true; status.textContent = error.message; });
